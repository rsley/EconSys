...
module.exports = (Dashboard) => {
  logger(
    "Success",
    "Dashbot",
    "Initialized Dashbot and globalized the Dashboard."
  );
  Dashboard.DBDEvents.on(
    "guildSettingsUpdated",
    async ({ changes, user, guildId }) => {
      ...
      if (keyChanged.toLowerCase() === "prefix") {
        ...
        logger(
          "Info",
          "Dashbot",
          `Prefix for ${guildId} changed to ${newPrefix} by ${user.tag} (${user.id})`
        );
      }
    }
  );
